...
public class GestorClientes extends JFrame {
	...
	private void actualizarLista() {
		...
		for(int i = 0 ; i < lista.size(); i++) {
			...
			row[0] = nombre.getId();
			row[1] = nombre.getNombre();
			row[2] = nombre.getApellido();
			row[3] = nombre.getEmail();
			row[4] = nombre.getTelefono();
		}
		((DefaultTableModel)table.getModel()).addRow(row);
		//datos.addElement(nombre.getNombreCompleto());
		//table.setModel((TableModel) datos);
		//((DefaultTableModel)table.getModel()).addRow(row);
	}
	...
	public GestorClientes() {
		...
		btnEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int indice = table.getSelectedRow();//Esto retorna el índice seleccionado
